...
class Protein:
    ...
    def from_pdb_string(
        ...
        Args:
            ...
            from_rosetta (bool, optional): Boolean specifying whether to use conversions between 
                Rosetta and OpenFold H atom naming. Defaults to False.
        ...
        for chain in sorted(model, key=lambda x: x.id):
            ...
            for res in sorted(chain, key=lambda x: x.id[1]):
                ...
                for atom in res:
                    # Convert from Rosetta to OpenFold H atom naming
                    if from_rosetta and ("H" in atom.name) and (not discard_Hs):
                        if atom.name not in restype_name_to_atom_name[res.resname]:
                            atom.name = rc.h_atom_rosetta_to_openfold[atom.name]
    ...
    def __hash__(self) -> int:
        """
        Need to define a hash function to identify unique Protein objects.
        Generates a unique integer based on the specific data in this instance of Protein.
        ...
        Returns:
            int: Unique integer hash of the Protein object.
        """
