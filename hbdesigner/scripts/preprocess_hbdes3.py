...
def dict_to_protein(data: Dict) -> Protein:
    """Convert from ProteinMPNN dict format to HBDesigner2 compatible Protein format."""
    ...
    # b_factors = torch.zeros_like(atom14_mask)
...
def preprocess_protein(
    ...
    """Preprocess a PDB file by idealizing sidechains and extracting HBNet matrix.
...
def _preprocess_pdb(entry: str, args: argparse.Namespace):
    # Get assembly metadata
    pdb_loc = os.path.join(args.data_dir, "pdb/")
    ...
    # Pack PDB metadata into tarfile for later
    subdir = os.path.join(args.out_file, entry[1:3])
    ...
    for asmb_i in sorted(set(asmb_ids)):
        ...
        # skip any that exceed max PDB chain number
...
def main(args):
    ...
    # Load metadata
    df = load_metadata(args.data_dir, args.split)
    ...
    with ProcessPool(max_workers=args.num_workers) as p:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        help="Directory containing ProteinMPNN dataset and metadata.",
    )
    parser.add_argument(
        "--out_file",
        type=str,
        help="Prefix of tarball that will contain final dataset files, Protein .npz files and .gml files.",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
